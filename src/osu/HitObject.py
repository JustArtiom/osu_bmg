...
class SliderObjectParams:
  ...
  def load_raw(self, raw: str):
    segments = [segments.strip() for segments in raw.split(",")]
    ...
    self.edge_sets = [tuple(map(int, edge_set.split(":"))) for edge_set in segments[4].split("|")]
